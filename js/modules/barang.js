...
async function renderPeminjaman(container) {
  ...
  // Fallback: jika flat kosong, coba ambil semua lalu filter di client
  let flat = res.flat || [];
  if (!flat.length && res.data) {
    const all = [];
    Object.values(res.data).forEach(arr => all.push(...arr));
    flat = all.filter(b => {
      const v = b.HanyaPinjam;
      return v === true || v === 'TRUE' || v === 'true' || v === 1 || v === '1';
    });
  }
  // Fallback kedua: ambil semua tanpa filter
  if (!flat.length) {
    const res2 = await api('getBarang', {});
    const all = res2.flat || [];
    flat = all.filter(b => {
      const v = b.HanyaPinjam;
      return v === true || v === 'TRUE' || v === 'true' || v === 1 || v === '1';
  ...
  for (const [kat, items] of Object.entries(grouped)) {
    html += `<div class="group-header">${kat}</div>`;
    items.forEach(b => {
      const stok = Number(b.Jumlah) || 0;
      html += `
        <label class="check-item">
          <input type="checkbox" data-id="${b.ID}" data-nama="${b.Nama}" data-max="${stok}" ${stok <= 0 ? 'disabled' : ''} />
          <div class="info">
            <div class="name">${b.Nama}</div>
            <div class="meta">${b.Jenis || ''} · <strong>Stok: ${stok}</strong></div>
          </div>
          <input type="number" min="1" max="${Math.max(stok, 1)}" value="1" data-qty="${b.ID}"
            style="width:64px;padding:4px;border:1px solid #ddd;border-radius:6px" ${stok <= 0 ? 'disabled' : ''} />
        </label>`;
    });
  }
  el.innerHTML = html || '<div class="empty">Tidak ada barang bertanda Hanya Pinjam. Cek kolom HanyaPinjam di spreadsheet.</div>';
...
async function renderPengambilan(container) {
  ...
  let res = await api('getBarang', { hanyaPinjam: 'FALSE' });
  let flat = res.flat || [];
  if (!flat.length) {
    res = await api('getBarang', {});
    flat = (res.flat || []).filter(b => {
      const v = b.HanyaPinjam;
      return !(v === true || v === 'TRUE' || v === 'true' || v === 1 || v === '1');
    });
  }
  ...
  for (const [kat, items] of Object.entries(grouped)) {
    ...
    items.forEach(b => {
      ...
      html += `<label class="check-item">
        <input type="checkbox" data-id="${b.ID}" data-nama="${b.Nama}" ${stok <= 0 ? 'disabled' : ''} />
        <div class="info"><div class="name">${b.Nama}</div><div class="meta">${b.Jenis || ''} · <strong>Stok: ${stok}</strong></div></div>
        <input type="number" min="1" max="${Math.max(stok, 1)}" value="1" data-qty="${b.ID}" style="width:64px;padding:4px;border:1px solid #ddd;border-radius:6px" ${stok <= 0 ? 'disabled' : ''} />
      </label>`;
    });
  }
