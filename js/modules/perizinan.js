...
async function ajukan() {
  ...
  if (!detail.tglMulai || !detail.tglKembali || !detail.keperluan) {
    toast('Tanggal mulai, tanggal kembali, dan keperluan wajib diisi', 'error');
    return;
  }

  // Selalu tanam detail ke setiap siswa (shared atau tidak, form tunggal dipakai bersama)
  // sharedDetail juga dikirim agar backend punya fallback
  const siswa = [...selected.values()].map(s => ({
    nisn: s.nisn,
    nama: s.nama,
    tglMulai: detail.tglMulai,
    jamMulai: detail.jamMulai || '',
    tglKembali: detail.tglKembali,
    jamKembali: detail.jamKembali || '',
  ...
  if (res.success) {
    ...
    document.getElementById('jam-mulai').value = '';
    document.getElementById('tgl-kembali').value = '';
    document.getElementById('jam-kembali').value = '';
